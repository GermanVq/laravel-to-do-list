                ...
                <template x-for="(task, index) in tasks" :key="index">
                    <div class="border-2 border-black px-5 py-4 rounded-2xl mb-8" :class="{'bg-green-100': task.completed, 'bg-red-100': !task.completed}">
                        <div class="flex justify-between">
                            <div class="text-gray-600">Fecha de creación: <span x-text="task.created_at"></span></div>
                            <div x-data class="text-gray-600">
                                <button  @click="$wire.showConfirmation(task.id)"
                        ...
                        <div class="flex space-x-2 items-center mb-5">
                            <div x-data>
                                <input :checked="task.completed" :id="'checkbox-' + task.id" type="checkbox" @click="$wire.toggleTask(task.id, !task.completed)"
                                ...
                                <div class="font-semibold text-lg" :style="task.completed ? 'text-decoration: line-through': ''" x-text="task.title"></div>
                            </div>
                        </div>
                        <div class="text-gray-600">
                            <p :style="task.completed ? 'text-decoration: line-through': ''" x-text="task.description"></p>
                        </div>
                    </div>
                </template>
            </div>
